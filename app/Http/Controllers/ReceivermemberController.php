...
class ReceivermemberController extends Controller {

	protected $layout = "layouts.main";
	protected $data = array();
	public $module = 'receivermember';
	static $per_page	= '50';

	public function __construct()
	{
		parent::__construct();
		$this->model = new Receivermember();

		$this->info = $this->model->makeInfo( $this->module);
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'receivermember',
    ...
    public function genMemberCode($year,$index = 1){
        ...
        $member_year = Receivermember::whereYear("start_date",$year)->get()->count();
        $member_year = $member_year + $index;
        $start_year = $year + 543;
        $padded_string = str_repeat("0", 6-strlen($member_year)) . $member_year;
        $newCode = "G" . $start_year . "_".$padded_string;

        $member = Receivermember::where("member_id",$newCode)->first();
        if ($member != null){
	...
	public static function display(  )
	{
		...
		$model  = new Receivermember();
		$info = $model::makeInfo('receivermember');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('receivermember.public.view',$data);
			}
		}
		else {
			...
			return view('receivermember.public.index',$data);
		}
